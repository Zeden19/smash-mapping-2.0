...
export const actions = {
  ...
  findPlayers: async ({request}) => {
    ...
    try {
      const response = await fetch(`https://api.smash.gg/players?filter={"gamerTag":%20"${playerTag}"}&page=1&per_page=20`);
      const {items: {entities: {player: players}}} = await response.json();
      const userIds = players.filter(player => player.hasUser).map(player => player.id);
      ...
      for (let i = 0; i < userIds.length; i++) {
        ...
        playerData.push({
          ...
          tournaments: player.user.tournaments.nodes
        });
      }
